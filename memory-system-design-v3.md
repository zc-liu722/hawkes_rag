...
> **一句话定位**
> 一个单一介质、turn 粒度、写入零 LLM 的记忆系统;白天靠 λ 使用动力学完成低生成-token 的召回、遗忘筛选与**实时矛盾分诊**,热/冷候选统一经 Qwen rerank 精排,夜间用一次低频 dreaming 只对高 λ 幸存者做巩固与矛盾**裁决**;三种召回源层级兜底,激活信号取"被采纳"而非"被召回"以根除 learning leak,矛盾信号经一个零 token 闸门稀疏触发、只在疑似矛盾时才花一次轻量调用,从而把负反馈补进控制回路而不破坏"日常少生成调用"。
...
超参数:$\beta$(按事实类型分档,需调);$\mu_{base}=0.1$、$k=\tfrac12$(有默认值,深入亦可调);$\tau_h$、$\tau_r$(候选触发与注入阈值,见第二部分 §3 与 §5);$\alpha$(冷召回 cosine/BM25 混合权重);$\theta_{flat}$(热库分布平缓触发阈值);`intermediate_top_k`(每轮取前 k 条视为成功调用);$\theta_c$(矛盾预筛触发阈值,新增,见 §4bis)。
...
> **硬性条款 · 存储形态(见第四部分 §3)**:embedding 几乎不变而 $\lambda$ 每轮在变,二者读写频率正交,因此底层是**混合存储**——向量索引负责 cosine ANN,可变 KV/列负责 $\lambda$ 与元数据,另配 BM25 文本索引服务冷召回词面通道。Qdrant 仍是向量与 payload 主存储,不引入 FAISS+sidecar。
>
...
2. **热召回——每轮隐式触发。** 每个 user turn 先用第一部分 score 公式在热路径取一批候选,再经过一次 rerank 后才进入阈值门控与注入。热库的 Hawkes score 负责"哪些记忆值得被看见",reranker 负责"这些候选里谁真的回答当前问题"。默认路径。
3. **冷召回——条件触发。** 当热召回信号不可靠时,绕过 $\lambda$ 的压制,在全池或冷池跑 cosine + BM25 混合候选召回,再接同一套 rerank,闭合"刻意找旧信息 / 查精确事实 / 查低频旧事实"的需求。旧信息从未删除,只是日常热路径里被 $\lambda$ 排到后面。

这里要区分两个概念:**召回候选分数**与**最终注入排序**。Hawkes score、cosine、BM25 都是便宜的候选生成器;最终进入上下文前统一经过 rerank,避免把不同量纲的分数直接相加。若 reranker 不可用,才退化为归一化分数或 RRF(Reciprocal Rank Fusion)合并。

### 3.1 热召回排序:先 Hawkes,再 Qwen rerank

热路径不是"取 top-k 直接塞上下文",而是:

1. 用 $\text{score}_i=\cos(q_i,m)[\mu+(1-\mu)\lambda^{-}(t_i)]$ 取 `hot_candidate_k` 个候选。
2. 对候选运行适配 Qwen 的 reranker,输入为 `(query, memory_text)` pair,输出 `rerank_score`。
3. 用 reranker 排序后的结果做注入阈值与自适应 k 判断。热路径保留原始 `hawkes_score`、`cos_at_recall`、`lambda_minus_snapshot` 作为后续激励、矛盾预筛和可观测字段,但注入顺序以 `rerank_score` 为准。

这样做的原因是热库负责快速召回,但 Hawkes score 仍是"语义相似度 × 使用动力学"的粗排分数;在精确问答、实体别名、否定语义、时间限定问题上,粗排 top-1 未必是最终应注入的 top-1。热库先 rerank 可以降低上下文污染,也让后续采纳信号更干净。

### 3.2 冷召回排序:cosine + BM25 混合候选,再 Qwen rerank

冷路径的目标不是"再跑一次纯语义",而是做精准查询。候选生成采用双通道:

- **cosine 通道**:适合语义改写、同义表达、跨语言/口语化查询。
- **BM25 通道**:适合姓名、地点、编号、日期、金额、型号、房间号、密码片段等精确词面匹配。

冷候选的第一阶段分数可用归一化加权:

$$
\text{cold\_candidate\_score}
=\alpha\cdot \operatorname{norm}(\cos(q,m))
+(1-\alpha)\cdot \operatorname{norm}(\operatorname{BM25}(q,m))
$$

默认 $\alpha$ 可从 0.5 起扫;若不想调分数尺度,可用 RRF 合并 cosine top-N 与 BM25 top-N。第一阶段只负责取 `cold_candidate_k` 个候选,不直接决定注入。第二阶段统一送入 Qwen reranker 重排。

Qwen 适配建议分两层:

- **Embedding**:继续用冻结的 Qwen embedding 产出 cosine,满足 C-2。
- **Rerank**:接 Qwen reranker/cross-encoder 接口,输入 query-passage pair,输出可比较的相关性分。评测中 reranker 型号也必须钉死,并作为控制变量记录;热、冷、热冷合并必须使用同一个 reranker,否则分数不可比。

### 3.3 热库到冷库的转换逻辑

冷召回不是每轮都跑,也不只由一个 top-1 阈值决定。触发条件应同时覆盖"热库没信心"与"热库结果太平均"两类情况:

1. **低置信触发**:热召回 rerank 后 top-1 低于注入阈值 $\tau_r$,或 Hawkes top-1 原始分低于 $\tau_h$。含义是热库没有找到足够像答案的记忆。
2. **分布均匀触发**:热候选分数过于平缓,例如 top-1 与 top-k 均值差距小、top-1/top-2 margin 小,或热候选 rerank softmax 熵高于阈值 $\theta_{flat}$。含义是热库"都有点像但谁都不确定",适合去冷库查精确证据。
3. **覆盖不足触发**:热路径过阈值候选数少于最小注入数,或热候选实体/时间跨度单一,不足以回答多跳、多会话或明确追问旧事实的问题。
4. **显式旧信息触发**:query 中出现"以前/上次/最早/之前说过/具体编号/哪一天/哪个房间"等强查旧信号时,即使热库有结果,也允许并行触发冷库。

触发后流程是:热库候选保留,冷库补取候选,两边去重后**一起进入同一个 Qwen reranker 做最终排序**。最终注入不按"热优先、冷次之"拼接,而按 rerank 后的统一相关性排序;每条仍保留 `retrieval_pool=hot|cold|both` 供观测与 ablation。若冷库未触发,只对热候选 rerank 后注入。
...
这是把负反馈(第一部分 §5)接入日间回路的机制。设计上必须同时满足两个约束:**(a) 不损害主 agent 回答主任务的注意力**;**(b) 不违反"日常少生成调用"与 §7 无子 agent 原则**。
...
- **旁路独立 LLM 不可取**:每个 user turn 多一次生成调用,违反 §7 判据(矛盾判定是单步集合标注,非自主多步有状态推理),并亲手破坏"日常少生成调用"。
...
- **对少生成调用 / 无子 agent 顾虑**:它不是旁路独立 agent,是主 agent 的第二次调用,且绝大多数 turn 不触发(预筛稀疏)。"日常少生成调用"在统计意义上保住——不是每轮加生成调用,只在预筛报警的稀疏 turn 付一次小调用。它依然是单步、无状态(喂候选→返回编号,无跨轮记忆),不构成 §7 中"需要独立 agent"的情形;是中间件触发的一次确定性子程序,不是一个 agent。
...
$\theta_c$ 是新增超参,与系统已有的 $\tau_r$、`intermediate_top_k` 同构,是本设计本就接受的旋钮类型,非新范式。

## 5. 注入策略:有效且不污染上下文

**分区放置**:MEMORY.md 放 system prompt 顶部(权威、稳定);热/冷候选经统一 rerank 后的片段放当前 turn 正上方,明确标记包裹(检索来的、未必全相关、可不用);冷召回触发时临时注入,用完即走,不进对话历史。

**阈值是抗污染总闸**:低于注入阈值 $\tau_r$ 的片段一律丢弃。热路径的 Hawkes score 先把"语义又不像、又早被遗忘"的片段压出候选池,reranker 再做最后相关性过滤;冷路径则由 cosine + BM25 扩大召回面,再由同一 reranker 收紧精度。

**注入量随 rerank 分布自适应,非固定 k**:最高分断崖领先 → 只塞 1–2 条;分数平缓但整体高 → 多塞几条让模型自挑;全部低于 $\tau_r$ → 一条不塞。"该沉默时能沉默"是区别于 RAG(永远塞 top-k)的能力。
...
相对主流系统:Mem0/Zep 实时抽取记忆(高频 token、有抽取无演化或演化重型),本机制延迟到夜间且只抽幸存者;Generative Agents 有 reflection 但需全量反思且 importance 静态,本机制用 $\lambda$ 先做零成本筛选、importance 由使用动态涌现;Letta 处处用 LLM 管记忆且记忆在 agent loop 内自编辑,本机制日常无额外生成式 LLM 管记忆、记忆是 loop 外的确定性中间件、仅 dreaming 一天进场一次。**矛盾处理上**:主流系统要么不处理更新(纯 RAG),要么用实时 LLM 抽取/裁决(Mem0 的 add/update/delete、Zep 的时序图重建),成本高;本机制用零 token 预筛把矛盾判定从"每轮都做"压缩为"稀疏触发",负反馈入回路而日常成本曲线几乎不动。
...
- **LongMemEval**(500 题,六类:单会话用户回忆、单会话助手回忆、偏好回忆、知识更新、时序推理、多会话综合)——**主战场**。S 变体每题仅 1–3 个相关 session 埋在 45+ 干扰 session 中,大海捞针设置,正面检验冷召回兜底 + $\tau_r$ 抗污染;且独有 abstention 类,正对本机制"该沉默时能沉默"相对纯 RAG 的固有优势。
...
| 日常少生成调用,框架不得塞自己的 LLM 记忆机制 | 只有 checkpointing,无语义记忆——对"记忆"零意见,不竞争、不污染 |
...
- **recall_node**:RecallMiddleware,确定性候选召回 + 非生成式 rerank。实现第一部分 score / μ-熵、热库 Qwen rerank、冷库 cosine+BM25 混合候选、热冷统一 rerank、$\tau_r$ 与自适应 k。LangGraph 节点本质是纯函数,完美承载"无状态中间件"。
- **main_llm_node**:唯一常规生成式 LLM 节点。第一次生成纯净;内部用 PydanticAI 约束输出契约。
...
| abstention | $\tau_r$ 阈值 + rerank 后自适应 k | 固定 top-k 不设 $\tau_r$(退化纯 RAG 注入) |
| multi-session | 冷召回兜底 + MEMORY.md 常驻 | 关冷召回 / 关 BM25 / 关 rerank / 关 MEMORY.md |
...
除准确率外,**必须报每 query 平均 LLM 调用数与 token 数**,与 Mem0/Zep 直接对比。本机制命脉是"日常少生成 token":主回答之外的生成调用只在 dreaming 与稀疏矛盾触发时发生。reranker 作为非生成式相关性模型,单独报告每 query rerank 候选数、rerank latency 与是否触发冷库。准确率 / token / rerank latency 这条曲线是相对 §8 对手最有说服力的图,不可只报准确率把它埋掉。
...
## 1. 三个生成式 LLM 调用点 + 一个 rerank 端口,不是一个模型

`main_llm`、`contradiction_micro`、`dreaming` 在 cost / latency / capability 上正交;`reranker` 是非生成式相关性模型,也要独立配置与钉版本:
...
| reranker(热/冷重排) | 中 | query + 候选片段 | 每轮热路径;冷触发时加冷候选 | Qwen reranker,本地优先 |
...
## 3. Embedding 与 Rerank 端口(独立两根线)

score 的 cos、cosine ANN、矛盾 $s_c$ 全由 embedding 驱动;最终注入排序由 reranker 驱动。二者功能不同,不得混成一个"检索模型"配置。
...
> **硬性条款 C-2bis · reranker 整 run 冻结**:热召回、冷召回、热冷合并必须使用同一个 Qwen reranker 型号与同一套归一化方式。否则热库与冷库最终分数不可比,也无法判断性能变化来自记忆机制还是 reranker 漂移。

Rerank 端口同样优先本地 sentence-transformers / transformers cross-encoder 形态。接口只需 `score(query, passages)->list[float]`;实现上可先支持 Qwen reranker,并提供一个无模型 fallback:按归一化 Hawkes/cold score 或 RRF 排序,用于离线快速 ablation。

## 4. 存储:混合形态 + 字段定义

复用件:**Qdrant**(单组件:向量索引做 cosine ANN + payload 存可变 λ 与元数据 + namespace 隔离 + 标量过滤),外加同 namespace 的 BM25 文本索引(可先用轻量内存/SQLite BM25,生产再换 Tantivy/Elasticsearch/OpenSearch)。每条记录字段:
...
bm25_text     : str            # 与 text 同步的分词/归一化文本,供冷召回词面检索
```
...
retrieved_segments  : list[{
    id, text, retrieval_pool,
    cos_at_recall, bm25_at_recall,
    hawkes_score, cold_candidate_score, rerank_score,
    lambda_minus_snapshot
}]
...
cold_trigger_reason : str | null
session_id          : str
namespace           : str
```

- 采纳判定(方案 C 的启发式路):最终回答与每条注入片段的**归一化 token-set 重叠或 embedding 相似度**,$\ge\theta_a$ 即采纳;$\theta_a$ 入附录超参表。
- 矛盾预筛信号仍以 `cos_at_recall` 为主:$\text{prescreen}=\max\{\text{cos\_at\_recall}_j \mid j\notin \text{adopted\_ids}\}$。rerank 负责相关性排序,但负向更新公式的 $s_c$ 仍使用 embedding cosine,保证第一部分公式闭合且不受 reranker 尺度影响。
- 冷库触发诊断字段记录"热库分布均匀 / score 未达阈值 / 显式旧信息"到底是哪一类原因触发,否则后续 ablation 只能看到冷库开没开,看不到转换逻辑是否正确。
...
每个 LLM 调用按 **(call-site, query-id)** 打点 token/latency;reranker 单独打点 latency、候选数与模型名;每轮记录 injected/adopted/contradicted 的 id、冷召回是否触发、触发原因、热/冷候选数、预筛是否跳闸。复用件:LiteLLM cost callback + **Langfuse**(或纯 OpenTelemetry)做 trace。无此层则第三部分 §6 的"准确率 / token"曲线画不出来。
...
| Rerank 端口 | Qwen reranker / cross-encoder(钉版本,本地优先) | 精准召回刚需(C-2bis) |
| 向量检索 + 可变 λ payload + namespace | **Qdrant**(单组件) | 刚需 |
| BM25 词面检索 | 内存/SQLite BM25 起步;生产 Tantivy/ES/OpenSearch | 冷库精准查询刚需 |
...
| $k$ | μ 曲率 | 1/2,可调 |
| $\tau_h$ | 热召回 Hawkes 候选触发阈值 | 需调;低于时触发冷库 |
| $\tau_r$ | rerank 后注入阈值(抗污染总闸) | 需调 |
| $\alpha$ | 冷召回 cosine / BM25 混合权重 | 默认 0.5 起扫;RRF 模式可不用 |
| $\theta_{flat}$ | 热库分布平缓触发阈值 | 需调;可由 margin 或 entropy 实现 |
| `intermediate_top_k` | 每轮取前 k 条视为成功调用 | 需调,过小激励过集中、过大无差别激活 |
| `hot_candidate_k` | 热路径进入 reranker 的候选数 | 需调;通常大于最终注入 k |
| `cold_candidate_k` | 冷路径 cosine+BM25 合并后进入 reranker 的候选数 | 需调 |
| $\theta_c$ | 矛盾预筛触发阈值(新增) | 需调,与 $\tau_r$、`intermediate_top_k` 同构 |
| $\theta_a$ | 采纳判定重叠阈值(新增) | 回答与片段归一化 token-set / embedding 重叠 ≥ $\theta_a$ 即判采纳;eval 敏感旋钮,需调 |

**评测控制变量(整 run 内冻结,不参与调参)**:backbone LLM(三方对比与 ablation 须同一型号,C-1);embedding 模型 + 维度(中途换则历史 λ 加权 cos 全失效,C-2);Qwen reranker 型号 + 归一化方式(C-2bis);`Clock` 取数据集时间(C-3)。这些不是超参,是评测正确性前提,违反则结果无效且不可察觉。

**敏感性分析计划(投稿用)**:需扫描——$\beta$ 分档值、$\tau_h$、$\tau_r$、$\alpha$、$\theta_{flat}$、`intermediate_top_k`、`hot_candidate_k`、`cold_candidate_k`、$\theta_c$;固定默认即可——$\mu_{base}$、$k$、$\theta_a$(报一次取值与一次 ±0.05 鲁棒性即可,不必全扫)。
